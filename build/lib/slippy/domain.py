...
class RectangularSphericalSection(Domain):
    ...
    def plot(self, plot_simulation_domain=False, projection = 'moll', ax=None):
        if ax is None:
            ax = plt.gca()
        plt.subplots_adjust(left=0.05, right=0.95)

        # Use a global plot for very large domains.
        cenlon = 0.5*self.center[0]
        if projection is 'moll':         
            m = Basemap(projection='moll', lon_0=cenlon, resolution="c", ax=ax)
            stepsize = 45.0
        # Orthographic projection for 75.0 <= extent < 180.0
        elif projection is 'ortho':
            m = Basemap(projection="ortho", lon_0=cenlon,
